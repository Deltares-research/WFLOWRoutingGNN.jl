...
#   2. Runs a Leslie-Smith LR range test at horizon 1 (the most stable /
#      trustworthy horizon) to pick the peak learning rate `lr_start`.
...
#   --lr-peak-decay X       curriculum peak-shrink factor  (default: 0.8)
...
function autotune(config::AbstractString, opts::AbstractDict)
    ...
    range_horizon = parse(Int,     optget(opts, "range-horizon", "1"))
    mem_fraction  = parse(Float64, optget(opts, "mem-fraction", "0.9"))
    peak_decay    = parse(Float64, optget(opts, "lr-peak-decay", "0.8"))
    ...
    if haskey(opts, "lr-start")
        ...
        # `min_over_10` (min-loss LR / 10) is the robust choice; the scalar
        # steepest-descent estimate is biased by the range test's EMA smoothing.
        lr_start = rec.min_over_10
        @info @sprintf("Range test → lr_start=%.3e  (steepest-descent alt=%.3e)",
                       lr_start, rec.steep)
        free_gpu!()
    end

    # --- 3. Option-1 curriculum knobs --------------------------------------
